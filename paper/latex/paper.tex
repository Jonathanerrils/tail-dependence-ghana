...
Three results carry this paper's argument. First, across the full sweep
of ten pairs, three quantile levels, two tails and three stress-window
definitions -- ninety simultaneous tests in total -- \textbf{zero cells
survive correction for multiple testing by either a family-wise
Bonferroni or a Benjamini--Hochberg procedure}. Tail dependence among
cocoa, gold and crude oil does not intensify detectably during stress in
this sample, whether stress is defined as the COVID shock, the 2024
cocoa episode, or their combination. Second, \textbf{expanding the
candidate copula family set from four to eight does not rescue the six
pairs -- all four commodity-commodity pairs plus Brent--WTI -- whose
dependence structure the narrower set already could not fit}: every one
of eight families is rejected by formal goodness-of-fit testing for all
six. This is evidence of a genuine model-specification gap, not of
absent dependence, since the same six pairs show substantial, precisely
estimated empirical tail concentration. Third, \textbf{the cedi's own
extreme-value profile is the most severe in the panel by a wide margin}
(GPD shape $\hat\xi = 0.67$ against 0.04--0.18 for the four commodities)
and resists six independent, increasingly sophisticated attempts at
adequate marginal modelling -- yet the substantive tail-dependence
conclusions are stable across all six, which is what licenses treating
them as robust despite the cedi's marginal imperfection rather than as
an artifact of any one modelling choice.

\subsection{Marginal models and univariate extremes}

Cocoa, gold and WTI pass all three adequacy diagnostics after the
specification search (WTI via EGARCH rather than the GJR default); Brent
and the cedi fail every specification tried and are retained at
best-AIC, with the failure disclosed. Table~\ref{tab:evt} reports
extreme-value results: WTI is the only commodity whose 95\% confidence
interval for the GPD shape excludes zero ($\hat\xi = 0.18$); the cedi's
shape, $\hat\xi = 0.67$, is roughly four times the heaviest commodity
tail, a severity gap corroborated by an independent block-maxima GEV
fit resting on a different asymptotic theorem.

\begin{table}[htbp]
\centering
\caption{Extreme value results: POT/GPD shape estimate (90th-percentile threshold) and implied tail risk measures, standardized-residual scale.}
\label{tab:evt}
\begin{tabular}{lrrr}
\toprule
Series & $\hat\xi$ & VaR$_{99}$ & ES$_{99}$ \\
\midrule
Cocoa & 0.050 & 2.68 & 3.41 \\
Gold  & 0.037 & 2.80 & 3.58 \\
Brent & 0.084 & 2.94 & 3.83 \\
WTI   & 0.182 & 2.90 & 3.96 \\
Cedi  & \textbf{0.671} & \textbf{6.56} & \textbf{20.18} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Copula selection and the model-risk finding}
\label{sec:results-gof}

Table~\ref{tab:gof8} reports, for each pair, the AIC-winning family among
all eight candidates and the count not rejected by the goodness-of-fit
test. The result is stark: six pairs -- every commodity-commodity pair
and Brent--WTI -- reject all eight families outright, none surviving at
the 5\% level. This is not a symptom of weak or absent dependence: with
roughly 2,900--3,000 daily observations per pair, the test has ample
power, and empirical lower-tail concentration for these pairs ranges
from 0.09 to 0.98 (Brent--WTI). It is evidence that the \emph{shape} of
commodity-commodity dependence is not well captured by any of eight
standard parametric families, including two families -- Frank and
Joe -- and two rotations rarely included in applied tail-dependence
studies. The four cedi pairs (excluding Cocoa--Cedi) show the opposite
pattern: all eight families simultaneously not rejected, consistent
with low statistical power against a weak signal, compounded by the
cedi's own marginal-adequacy problem. Cocoa--Cedi is intermediate:
two of eight families are not rejected.

\begin{table}[htbp]
\centering
\caption{Eight-family copula comparison: AIC winner and goodness-of-fit survival.}
\label{tab:gof8}
\small
\begin{tabular}{lccc}
\toprule
Pair & AIC-winning family & $\hat\lambda_L$ (empirical, $q=0.05$) & Families not rejected (of 8) \\
\midrule
Cocoa--Gold  & $t$               & 0.093 & 0 \\
Cocoa--Brent & survival Gumbel   & 0.167 & 0 \\
Cocoa--WTI   & survival Gumbel   & 0.140 & 0 \\
Cocoa--Cedi  & Gaussian          & 0.140 & 2 \\
Gold--Brent  & $t$               & 0.127 & 0 \\
Gold--WTI    & $t$               & 0.140 & 0 \\
Gold--Cedi   & Gaussian          & 0.173 & 8 \\
Brent--WTI   & $t$               & 0.979 & 0 \\
Brent--Cedi  & $t$               & 0.213 & 8 \\
WTI--Cedi    & Gaussian          & 0.180 & 8 \\
\bottomrule
\end{tabular}
\end{table}

Notably, adding the four new families \emph{did} change which family
wins by AIC for two pairs -- Cocoa--Brent and Cocoa--WTI now prefer the
survival Gumbel copula, revealing genuine lower-tail (crash) dependence
($\hat\lambda_L \approx 0.07$ analytically) that the narrower four-family
set's AIC-preferred choices did not surface -- but the survival Gumbel
fit is itself rejected by the goodness-of-fit test for both pairs. A
richer candidate set changed which shape looked \emph{least} inadequate
without changing the finding that none is adequate, which is precisely
the distinction between AIC selection and absolute goodness-of-fit that
motivates including both in this analysis.

\subsection{Stress-versus-calm dependence, fully corrected}
\label{sec:results-calmstress}

Table~\ref{tab:calmstress-means} reports mean empirical lower- and
upper-tail concentration across all ten pairs, calm versus the combined
COVID+2024 stress window, at each tested quantile. At every quantile and
in both tails, the calm-to-stress change is small relative to the level
and inconsistent in sign -- the signature of sampling noise around a
null effect, not a strengthening dependence structure.

\begin{table}[htbp]
\centering
\caption{Mean empirical tail concentration across all ten pairs, calm vs.\ combined stress, by quantile and tail.}
\label{tab:calmstress-means}
\begin{tabular}{llrrr}
\toprule
$q$ & Tail & Calm & Stress & Change \\
\midrule
0.025 & lower & 0.104 & 0.115 & $+0.011$ \\
0.025 & upper & 0.102 & 0.069 & $-0.033$ \\
0.050 & lower & 0.143 & 0.149 & $+0.006$ \\
0.050 & upper & 0.132 & 0.103 & $-0.029$ \\
0.100 & lower & 0.192 & 0.183 & $-0.009$ \\
0.100 & upper & 0.190 & 0.169 & $-0.021$ \\
\bottomrule
\end{tabular}
\end{table}

Of sixty cells tested at the combined-stress definition, seven are
nominally significant uncorrected (all decreases, all involving the
cedi); \textbf{zero survive family-wise Bonferroni correction, and zero
survive Benjamini--Hochberg FDR correction}. Disaggregating the stress
definition (Table~\ref{tab:disagg}) shows the two episodes behave
differently in level, consistent with their different economic
character, but the correction result is identical either way: zero
cells survive either correction under COVID-only stress or under
2024-only stress. One methodological note earned through direct
verification rather than assumption: an initial pass at 500 bootstrap
replicates suggested six COVID-only cells survived Bonferroni, all with
$p$-values sitting exactly at the resolution floor a 500-replicate
bootstrap can produce; re-estimating those specific cells at 5,000 and
15,000 replicates resolved all six to non-significant, confirming the
zero-survivor result rather than overturning it, and illustrating
precisely the kind of artifact a coarse bootstrap can manufacture at
stringent correction levels.

\begin{table}[htbp]
\centering
\caption{Disaggregated stress windows: mean tail concentration, calm baseline held constant.}
\label{tab:disagg}
\small
\begin{tabular}{llrrr}
\toprule
Window & Tail & $q$ & Calm & Stress \\
\midrule
COVID-only & lower & 0.05 & 0.143 & 0.161 \\
COVID-only & upper & 0.05 & 0.132 & 0.023 \\
2024-only  & lower & 0.05 & 0.143 & 0.137 \\
2024-only  & upper & 0.05 & 0.132 & 0.115 \\
\bottomrule
\end{tabular}
\end{table}

COVID shows a pattern consistent with a broad-based crash -- lower-tail
commodity dependence rises modestly while upper-tail dependence
involving the cedi falls sharply -- while 2024 shows no comparable
pattern in either tail, consistent with a supply-driven, comparatively
idiosyncratic price surge rather than a systemic shock. Neither pattern
survives correction, so we report the qualitative contrast
descriptively rather than as a statistical finding.

\subsection{Dynamics and networks}

A rolling 250-day $t$-copula (four pairs: Cocoa--Gold, Cocoa--Brent,
Cocoa--Cedi, Brent--Cedi) shows no visible stress-linked surge in
correlation for either cedi pair examined ($\hat\rho_t \in [-0.15,
0.17]$ and $[-0.11, 0.11]$ respectively across 132 rolling windows).
Brent--WTI, not included in that rolling analysis, is covered instead by
a second, nonparametric rolling estimator spanning all ten pairs:
its empirical $\hat\lambda_L$ ranges from 0.56 to 0.88 across 2,748
daily-stepped windows, consistently the highest of any pair and without
a stress-linked spike -- a structurally elevated baseline, not
contagion. Combined with the static full-sample fit ($\hat\rho=0.945$)
and calm-to-stress empirical estimates ranging from 0.72 to 0.92
depending on the stress-window definition used, Brent--WTI is the single
most thoroughly and consistently evidenced pattern in this paper across
every estimator applied to it -- expected of two close substitutes, and
unrelated to the stress-contagion hypothesis under test.

\subsection{Transmission to Ghana macroeconomic variables}

Of nine regressor-target combinations (three lagged commodity
joint-tail-activity measures against cedi depreciation, CPI change, and
export growth), one is significant at 5\% uncorrected: lagged rolling
Cocoa--Brent tail dependence predicting export growth ($p=0.011$,
$R^2=0.069$, $n=88$). Placed against the roughly ninety hypothesis tests
conducted in Section~\ref{sec:results-calmstress} alone, one nominal hit
is close to the number expected under pure chance and survives no
multiple-testing correction; we report it for completeness and treat it
as noise rather than a transmission channel. Cedi depreciation and CPI
change show no significant relationship to any commodity joint-tail
measure ($R^2 \approx 0.001$--$0.005$).
