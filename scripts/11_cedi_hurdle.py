...
if order == 1:
    X = sm.add_constant(pd.DataFrame({"I1": I_lag1}))
else:
    X = sm.add_constant(pd.DataFrame({"I1": I_lag1, "I2": I_lag2}))
p_t = best_logit.predict(X.dropna())
p_t.name = "p_zero"

# LR test vs a constant-probability (i.i.d. hurdle) null: correct null
# log-likelihood is the SUM of per-observation Bernoulli log-likelihoods
# at p_hat = sample mean, NOT a binomial PMF (which wrongly adds a
# combinatorial term that has nothing to do with comparing these two
# sequence models and can make the LR statistic go negative).
k = df_logit["I"].sum()
n = len(df_logit)
p_hat = k / n
...
# ------------------------------------------------ Part 2: continuous submodel,
# fit on the NONZERO subsequence only, then mapped onto the full calendar
# with volatility HELD CONSTANT through zero-return stretches (zero days
# are treated as censoring -- BoG chose not to move the rate -- not as
# evidence of literally zero volatility, which is what caused Treatment
# A's conditional-vol path to collapse toward degenerate near-zero values
# during zero-return runs and destabilize the hurdle construction below).
nz = cedi[cedi != 0]
print(f"\nNonzero-only subsequence: {len(nz)} obs ({len(nz)/len(cedi):.1%} of {len(cedi)})")
# Static (non-time-varying) Student-t fit to the nonzero returns via MLE.
# This deliberately isolates the question this treatment exists to answer
# -- does EXPLICIT zero-mass modeling fix the marginal-adequacy problem
# -- from the separate, harder question of how a GARCH volatility
# recursion should treat zero-return days (censoring vs. zero-vol
# evidence), which repeated attempts above showed interacts badly with
# the hurdle construction. A static continuous part is a legitimate,
# common hurdle-model simplification and a clean baseline; a time-varying
# version is noted as unresolved future work rather than forced to "work."
...
_res = _opt.minimize(_neg_ll, x0=[0.0, np.log(0.1), np.log(3.0)], method="Nelder-Mead")
...
F0 = np.full(len(common_idx), F0)  # constant, since the continuous part is static
...
with open("outputs/fits.pkl", "rb") as f:
    fits = pickle.load(f)
ks_before = stats.kstest(fits["cedi"].pit.dropna(), "uniform")
