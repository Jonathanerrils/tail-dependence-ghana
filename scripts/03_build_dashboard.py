...
def heatmap_panel() -> go.Figure:
    ...
    for i, m in enumerate([calm, s24], start=1):
        fig.add_trace(
            go.Heatmap(z=m.values, x=m.columns, y=m.index, zmin=0, zmax=0.7,
                       colorscale="YlOrRd", showscale=(i == 2),
                       text=np.round(m.values, 2), texttemplate="%{text}",
                       colorbar=dict(title="λ<sub>L</sub>")),
            row=1, col=i)
    fig.update_layout(height=420, font=dict(family=FONT),
...
def rolling_panel() -> go.Figure:
    ...
    keep = [c for c in roll.columns if "Cocoa" in c or "cocoa" in c or "brent" in c
            or "Brent" in c][:6] or roll.columns[:6]
    fig = go.Figure()
    for c in keep:
        fig.add_trace(go.Scatter(x=roll.index, y=roll[c], name=c, line=dict(width=1.4)))
    ...
    fig.update_layout(height=430, font=dict(family=FONT),
                      title="Rolling 250-day lower-tail dependence, λ̂<sub>L</sub>(q = 0.10)",
                      yaxis_title="λ̂<sub>L</sub>", legend=dict(orientation="h", y=-0.2))
    return fig


def timeline_panel(data: str = "synthetic") -> go.Figure:
...
def main(data: str = "synthetic") -> None:
    ...
    banner = ("" if data == "real" else
              "<p class='warn'>Demo run on <strong>synthetic</strong> data — "
              "run <code>scripts/01_fetch_real_data.py</code> and rebuild with "
              "<code>--data real</code> before interpreting results.</p>")
